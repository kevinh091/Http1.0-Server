...
void logger_raw(char* ipaddr, time_t recv_t, time_t exe_t, const char *request, int status, size_t contlen){
	...
	strftime(recv, sizeof(recv), "%d/%b/%Y:%H:%M:%S -%z", dt);
	dt = localtime(&exe_t);
	strftime(exe, sizeof(exe), "%d/%b/%Y:%H:%M:%S -%z", dt);
	sprintf(printmsg, "%s - [%s] [%s] %s %d %zu", ipaddr, recv, exe, request, status, contlen);
	if(DEBUG){
		std::cout << printmsg << std::endl;
	}else
	if(loggin){
		std::ofstream of(fs,  std::ios::out | std::ios::app );
...
void sendahead(struct client_req &creq, time_t service_time, int);
