...
void execute_request(){
	...
	while(!done){
		...
		if(creq.content_type==DIREC){
			...
			if(access(strcpy(content,((creq.file_name+((creq.file_name[creq.file_name.size()-1]=='/')?"index.html":"/index.html")).c_str())), F_OK ) == -1){
				status = 404;
				sendahead(creq, service_time, status);
				...
				std::string newnew(content);
				std::cout << creq.file_name << std::endl;
				std::cout << "content: " << content << std::endl;
				creq.file_name = newnew;
				struct stat status2;
				creq.file_stat = stat(content, &status2);
...
void sendahead(struct client_req &creq, time_t service_time, int status_code){
	char inist[20];
	sprintf(inist, "HTTP/1.0 %d\n", status_code);
	send(creq.socket, inist, strlen(inist), 0);
	...
	if(creq.content_type==DIREC){
		send(creq.socket, "text/html", 9, 0);
	}else{
		std::string temp;
		for(int i=1; i<5; i++){
			temp = temp+ creq.file_name.at(creq.file_name.size()-i);
		}
	if(temp =="lmth" || temp=="txt."){
		send(creq.socket, "text/html", 9, 0);
	}else {
		send(creq.socket, "image/gif", 9, 0);
	}
	}
	//std::string str = (creq.content_type==DIREC)? "text/html" :  ((creq.file_name.find(".html .txt", creq.file_name.size()-6, 4)==std::string::npos)? "image/gif":"text/html");
	...
	while (!done) {
		...
		for(std::pair<int, bool> pair : wcyifuyin21){
			...
			if (FD_ISSET(soc, &ready)) {
				...
				iss >> creq.req;
				char buf[CWDMAX];
				ERRORIF(getcwd(buf,CWDMAX)==NULL, "get cwd failed");
				std::string wrapper(buf);
				
				iss >> creq.file_name;
				if(creq.file_name.empty()){
					goto skippy;
				}
				if(creq.file_name.at(0)=='~'){
					std::string wrapper2(user_s_domain);
					wrapper = wrapper2;
					creq.file_name = creq.file_name.substr(1);
				}
				if(creq.file_name.empty()){
					goto skippy;
				}
				creq.file_name=wrapper+(creq.file_name.at(0)=='/'? creq.file_name : ("/"+creq.file_name));
				std::cout << creq.file_name << std::endl;
				iss >> creq.protocol;
				...
				if(S_ISDIR(status.st_mode)){
					std::string temp = creq.file_name +(creq.file_name.at(creq.file_name.size()-1)=='/'? "index.html":"/index.html");
					status.st_size = 0;
					stat(temp.c_str(), &status);
				}
